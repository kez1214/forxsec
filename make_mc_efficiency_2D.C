...
void make_mc_efficiency_2D() {
    ...
    auto hDen = df_den.Histo2D(
        {"hDen", "Reconstructed Candidates (all);|y|;p_{T} [GeV]",
         nYBins, yBins.data(), nPtBins, ptBins.data()},
        "denAbsY", "denPt"
    );

    // ============================================================
    // 分子：在分母基础上加 analysis 筛选
    // ============================================================
    auto df_ana = df_den.Define(   // 从 df_den 继承，保证事件一致
        "mumuCandidates",
        get_mumu_candidates,
        {
    ...
    std::cout << "\nDenominator entries (reco): " << hDen_ptr->GetEntries() << std::endl;
    std::cout << "Numerator entries (analysis): " << hNum_ptr->GetEntries() << std::endl;
